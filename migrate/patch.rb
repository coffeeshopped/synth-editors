...
def parms(content)
  content = content.gsub(/\.p\("(.*?)", (.*)\)/) do |match|
    path = $1
    obj = $2
    obj = obj.gsub(/\.max\((.*)\)/) do |m| "max: #{$1}" end
    obj = obj.gsub(/\.options\((.*)\)/) do |m| "opts: #{$1}" end
    obj = obj.gsub(/\.opts\((.*)\)/) do |m| "opts: #{$1}" end
    obj = obj.gsub(/\.rng\(dispOff:(.*)\)/) do |m| "dispOff:#{$1}" end
    obj = obj.gsub(/\.rng\((-?\d+)\.\.\.(\d+)\)/) do |m| "rng: [#{$1}, #{$2.to_i}]" end
    obj = obj.gsub(/\.rng\((-?\d+)\.\.\.(\d+), dispOff: (.*?)\)/) do |m| "rng: [#{$1}, #{$2.to_i}], dispOff: #{$3}" end
    obj = obj.gsub(/^0x(\w+)/) do |m| "b: 0x#{$1}" end
    "['#{path}', { #{obj} }]"
  end
...
enums = ['switcher', 'button', 'dimPanel', 'child', 'children', 'basicControlChange', 'basicPatchChange', 'setCtrlLabel', 'configCtrl', 'dimItem', 'simpleGrid', 'dimsOn', 'editMenu', 'setValue']
enums.each do |en|
