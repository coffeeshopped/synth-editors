...
const parms = [
  ...
  { prefix: "filter", count: 2, bx: 20, block: [
    ["type", 77, {opts: filterTypeOptions}],
    ["cutoff", 78],
    ["reson", 80],
    ["drive", 81],
    ["drive/curve", 82, {opts: driveCurveOptions}],
    ["keyTrk", 86, {iso: keytrackIso}],
    ["env/amt", 87, {dispOff: -64}],
    ["velo", 88, {dispOff: -64}],
    ["cutoff/src", 89, {opts: modSourceOptions}],
    ["cutoff/amt", 90, {dispOff: -64}],
    ["fm/src", 91, {opts: fmSourceOptions}],
    ["fm/amt", 92],
    ["pan", 93, {dispOff: -64}],
    ["pan/src", 94, {opts: modSourceOptions}],
    ["pan/amt", 95, {dispOff: -64}],
  ] },
  ["filter/routing", 117, {opts: ["Para", "Serial"]}],
  ["volume", 121],
  ["amp/velo", 122, {dispOff: -64}],
  ["amp/mod/src", 123, {opts: modSourceOptions}],
  ["amp/mod/amt", 124, {dispOff: -64}],
  { prefix: "fx", count: 2, bx: 16, block: i => (
    { inc: true, b: 128, block: [
      ["type", {opts: i == 0 ? effectTypeOptions : effect2TypeOptions}],
      ["mix"],
    ] }
  )} ,
  ...
  { prefix: "lfo", count: 3, bx: 12, block: [
    ["shape", 160, {opts: lfoShapeOptions}],
    ["speed", 161],
    ["sync", 163, {max: 1}],
    ["clock", 164, {max: 1}],
  ...
  { prefix: "env", count: 4, bx: 12, block: [
    ["mode", 196, {bits: [0, 3], opts: envelopeModeOptions}],
    ["trigger", 196, {p: -1, bit: 5, opts: envelopeTriggerOptions}],
    ["attack", 199],
    ["attack/level", 200],
    ["decay", 201],
    ["sustain", 202],
  ...
  { prefix: "modif", count: 4, bx: 4, block:
    { inc: true, b: 245, block: [
      ["src/0", {opts: modSourceOptions}],
      ["src/1", {opts: ["Const"].concat(modSourceOptions.suffix(1))}],
      ["op", {opts: modOperatorOptions}],
      ["const", {dispOff: -64}],
    ] }
  },
  { prefix: "mod", count: 16, bx: 3, block:
    { inc: true, b: 261, block: [
      ["src", {opts: modSourceOptions}],
      ["dest", {opts: modDestinationOptions}],
      ["amt", {dispOff: -64}],
    ] }
  },
  { prefix: "arp", block: [
    ...
    ["clock", 314, {opts: arpClockOptions}],
    ["length", 315, {opts: arpLengthOptions}],
    ["octave", 316, {max: 9, dispOff: 1}],
    ["direction", 317, {opts: arpDirections}],
    ["sortOrder", 318, {opts: arpSortOptions}],
    ["velo", 319, {opts: arpVeloModeOptions}],
    ["timingFactor", 320],
  ...
  { prefix: '', count: 16, bx: 1, block: [
    ["step", 327, {bits: 4...6, dispOff: -4, opts: arpStepOptions}],
    ["glide", 327, {bit: 3}],
...
const oscOctaveOptions = Array.sparse([
  ...
  const filterRoutingOptions = ["Parallel", "Serial"]
...
const modOperatorOptions = ["+", "-", "*", "AND", "OR", "XOR", "MAX", "min"]
  
const unisonModeOptions = ["off", "dual", "3", "4", "5", "6"]
