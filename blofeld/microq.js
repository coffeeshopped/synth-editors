...
function sysexHeader(deviceId) { 
  return [0xf0, 0x3e, 0x10, deviceId]
}

/// dumpByte: cmd byte for what kind of dump.
function sysexData(bytes, deviceId, dumpByte, bank, location) {
  return sysexHeader(deviceId).concat([dumpByte, bank, location], bytes, [0x7f, 0xf7])
}

// devID=127 is OMNI, but a PDF I read said 0 is default for sound designers
function fileData(bytes, dumpByte) {
  return sysexData(bytes, 0, dumpByte, 0x20, 0x00)
}
