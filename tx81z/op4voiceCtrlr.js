...
const opPath = (index) => {
  return (p) => {
    p[0] == 'extra' ? [.extra, .op, .i(index)] + p.subpath(from: 1) : [.voice, .op, .i(index)] + p
  }
}
...
const miniOpController = (index, ratioEffect, opType, allPaths) => {
  return {
    builders: [
      ['items', { color: 2 }, [
        [envItem(index), "env"],
    ...
    effects: [
      ...
      .editMenu("env", { paths: opPaths(index, allPaths), type: opType }),
    ], 
    layout: [
      ['row', [["op",1],["freq",4]]],
      ['row', [["env",1]]],
