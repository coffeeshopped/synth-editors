...
const parms = [
  {
    prefix: 'part', count: 8, bx: 12, block: (i) => [
      ...
      ["channel", { b: 3, .opts(16.map { "\($0 + 1)" } + ["Omni"]) }],
...
const patchChangeTransform = (patchWerk) => {
  return {
    ...
    param: (editorVal, bodyData, parm, path, value) => {
      if path.last() == 'number' {
        return [
          [['syx', patchWerk.paramData(editorVal, [parm.b - 1, value.bit(7)])], 50],
          [['syx', patchWerk.paramData(editorVal, [parm.b, value.bits(0, 7)])], 0],
        ]
      }
      else {
        return [[['syx', patchWerk.paramData(editorVal, [parm.b, bodyData[parm.b]])], 0]]
      }
    }, 
...
const sysexData = (bd, channel) =>
  yamahaSysexData(channel, [0x7e, 0x00, 0x78], "LM  8976PE".sysexBytes().concat(bd))
...
const bankSysexData = (bodyData, channel, patchCount, truss, compactTruss) => {
  return Op4.bankSysexData(bodyData, channel, patchCount, truss, compactTruss, [0x7e, 0x13, 0x0a], "LM  8976PM")
}
...
const createBankTruss = (patchCount, patchTruss, compactTruss, initFile) => {
  return {
    ...
    initFile: initFile,
    fileDataCount: 2450, 
    createFileData: (bd) => bankSysexData(bd, 0, patchCount, patchTruss, compactTruss), 
    parseBodyData: {
      type: 'yamahaCompactTrussWerkCompactParseBodyData', 
      parseOffset: 16, 
      patchTruss: patchTruss, 
      compactPatchTruss: compactTruss!, 
      patchCount: patchCount,
    },
  }
}
...
const wholeBankTransform = (patchCount, patchTruss, compactTruss) => {
  return {
    ...
    wholeBank: (editorVal, bodyData) =>
      [['syx', bankSysexData(bodyData, editorVal, patchCount, patchTruss, compactTruss)], 100],
  }
}
