...
function commonPatchWerk(parms) {
  return {
    ...
    //   "velo" : 64,
    // ] <<< 4.dict { ctrl in
    //   ["mtrx/ctrl/ctrl/src" : 0] <<< 4.dict(transform: { dest in
    //     [
    //       "mtrx/ctrl/ctrl/dest/dest" : 0,
    //       "mtrx/ctrl/ctrl/amt/dest" : 64,
    //     ]
    //   })
    // }
  }
...
// 
    ...
    // "amp/env/time/0" : (0...30).rand(),
    // "amp/env/time/1" : (30...40).rand(),
    // "amp/env/time/2" : (20...80).rand(),
    // "amp/env/time/3" : (0...80).rand(),
    // "amp/env/level/0" : 127,
    // "amp/env/level/1" : (30...127).rand(),
    // "amp/env/level/2" : (30...127).rand(),
// 
    // "lfo/0/pitch" : (54...84).rand(),
    // "lfo/1/pitch" : (54...84).rand(),
    // "bias/level" : 64,
  }
}

function patchWerk(werks, initFile) {
  return {
    multi: "Voice", 
    map: [
      ...
      ["tone/0", 0x2000, werks.tone],
      ["tone/1", 0x2200, werks.tone],
      ["tone/2", 0x2400, werks.tone],
      ["tone/3", 0x2600, werks.tone],
    ],
    initFile: initFile,
  }
}
...
function bankWerk(patchWerk) {
  return {
    multiBank: patchWerk,
    patchCount: 128,
    initFile: "xv5050-voice-bank-init",
  }
}
