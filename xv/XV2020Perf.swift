...
extension XV2020 {
  ...
  enum Perf {
    ...
    static let patchWerk = XV.Perf.patchWerk(16, common: XV5050.Perf.Common.patchWerk, part: Part.patchWerk, fx: FX.patchWerk, chorus: Chorus.patchWerk, reverb: Reverb.patchWerk, other: [], initFile: "xv2020-perf-init")
  
    static let bankWerk = XV.Perf.bankWerk(patchWerk, initFile: "xv2020-perf-bank-init")

    static let fullRefTruss = XV.Perf.Full.refTruss(16, perf: patchWerk, voice: Voice.patchWerk, rhythm: rhythmPatchWerk)

    enum Part {
      static let patchWerk = XV.Perf.Part.patchWerk(params: parms.params(), size: 0x31)
 
      static let parms = XV5050.Perf.Part.parms + [
        .p([.out, .assign], 0x1f, .options(outAssignOptions)),
        .p([.out, .fx], 0x20, .opts(["A"])),
      ]
      
      static let outAssignOptions: [Int:String] = XV2020.Voice.Tone.outAssignOptions <<<
        [13 : "Patch"]
      ...
      static let config = XV.Perf.Part.Config(voicePartGroups: voicePartGroups, rhythmPartGroups: rhythmPartGroups, voicePresets: voicePresets, rhythmPresets: rhythmPresets)

      static let voicePartGroups: [Int:String] = internalPartGroupOptions <<< XV.Perf.Part.srxPartGroupOptions
      
      static let rhythmPartGroups: [Int:String] = {
        let ids: [SynthPath] = [
          [],
          [.int, .user],
          [.int, .preset, .i(0)],
