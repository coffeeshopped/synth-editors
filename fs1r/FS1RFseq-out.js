...
const params: SynthPathParam = {
  ...
  (0..<512).forEach { step in
    ...
    p["pitch/step/step"] = RangeParam(parm: 0x10, byte: 0x10 + off, extra: [0:2], maxVal: 16383)

    (0..<8).forEach { trk in
      let offTrk = off + trk
      p["trk/trk/voiced/freq/step/step"] = RangeParam(parm: 0x12 + trk, byte: 0x12 + offTrk, extra: [0:2], maxVal: 16383)
      p["trk/trk/voiced/level/step/step"] = RangeParam(parm: 0x22 + trk, byte: 0x22 + offTrk)
      p["trk/trk/unvoiced/freq/step/step"] = RangeParam(parm: 0x2a + trk, byte: 0x2a + offTrk, extra: [0:2], maxVal: 16383)
      p["trk/trk/unvoiced/level/step/step"] = RangeParam(parm: 0x3a + trk, byte: 0x3a + offTrk)
    }
  }
...
extension FS1R {
  ...
  enum Fseq {
    ...
    enum Bank {
      ...
      const bankTruss = SingleBankTruss(patchTruss: patchTruss, patchCount: 6, createFileData: bankCreateFileData(sysexData), parseBodyData: bankParseBodyData(patchTruss: patchTruss, patchCount: 128), validBundle: (
        validSize: {
