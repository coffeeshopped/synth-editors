...
function createPatchTruss(createFileData) {    
  return {
    ...
    parseBody: (bodyData) => {
      let d = bodyData.safeBytes(offset: 5, count: 268)
      return 134.map((i) => {
        let off = i * 2
    ...
    unpack: (bodyData, param) => {
      ...
      let byte = param.byte
      guard byte < bodyData.count else { return nil }
...
function createBankTruss(patchTruss) {
  let patchCount = 100
  let parseBodyData = SingleBankTrussWerk.sortAndParseBodyDataWithLocationIndex(4, patchTruss: patchTruss, patchCount: patchCount)
  let createFileData = SingleBankTrussWerk.createFileDataWithLocationMap({ sysexData($0, location: $1) })
  // 30404 is a full dump with extra global/splits info
  return SingleBankTruss(patchTruss: patchTruss, patchCount: patchCount, createFileData: createFileData, parseBodyData: parseBodyData, validSizes: [30404], includeFileDataCount: true)
}

const voice = {
  
  patchTruss: createPatchTruss(createFileData: { sysexData($0, location: 0) }),

  createPatchTruss: createPatchTruss,
  
  sysex: function(bodyData, location) {
    ['syx', sysexData(bodyData, location)]
  },
  
  sysexData: function(bodyData, location) {
    sysexData(bodyData, [0x01, UInt8(location)])
  }
  
  sysexData: function(_ bodyData: [UInt8], withHeader header: [UInt8]) -> [UInt8] {
    let bodyBytes = bodyData.map { [UInt8($0.bits(0...3)), UInt8($0.bits(4...7))] }.joined()
    let checksum = UInt8(bodyData.map{ Int($0) }.reduce(0, +) & 0x7f)
    return Matrix6.sysex(cmdBytes: header + bodyBytes + [checksum])
  }
  ...
  bankTransform: {
    ...
    bank: (editorVal, bodyData, location) => {
      [(sysex(bodyData, location: location), 50)]
    },
  }
      

  static func patchTransform() -> MidiTransform {
    let patchOut: (Int, [UInt8]) -> (MidiMessage, Int) = {
      (sysex($1, location: $0), 100)
    }
    
    return .single(throttle: 200, Matrix6.tempPatch, .patch(param: { v, bodyData, path, value in
      ...
      if param.parm < 0 {
        ...
        if value < 0 || path == [.env, 0, .sustain] || path == [.amp, 1, .env, 1, .amt] {
          ...
          return [
            (.sysex(Matrix6.sysex(cmdBytes: [0x05])), 10), // quick edit mode bytes
            (.sysex(Matrix6.sysex(cmdBytes: [0x06, UInt8(param.parm), UInt8(value)])), 10),
          ]
        }
      }
      ...
      rampModeOptions: ["Single","Multi","External","Ext Gated"],
      fixedModsOptions: ["Off","Bend","Vibrato","Both"],
      
      mixOptions = 64.map((i) => {
        switch i {
        case 31: return "Equal"
        case 0..<31:
          return `O2 +${31 - i}`
        default:
          return `O1 +${i - 31}`
      ...
      portaOptions: ["Off","Porta","Key Track"],
      
      modSourceOptions: ["Unused","Env 1","Env 2","Env 3","LFO 1","LFO 2","Vibrato","Ramp 1","Ramp 2","Keyboard","Portamento","Tracking Gen","Keybd Gate","Velocity","Release Velo","Pressure","Pedal 1","Pedal 2","Lever 1 (Bend)","Lever 2 (Mod)","Lever 3 (Breath)"],
      
      static let trkSrcOptions: [Int:String] = {
        var opts = OptionsParam.makeOptions(modSourceOptions)
        opts.removeValue(forKey: 0)
        return opts
      }()
      
      modDestinationOptions: ["Unused","DCO1 Freq","DCO1 PW","DCO1 Waveshape","DCO2 Freq","DCO2 PW","DCO2 Waveshape","Mix Level","VCF FM Amount","VCF Freq","VCF Resonance","VCA1 Level","VCA2 Level","Env1 Delay","Env1 Attack","Env1 Decay","Env1 Release","Env1 Amp","Env2 Delay","Env2 Attack","Env2 Decay","Env2 Release","Env2 Amp","Env3 Delay","Env3 Attack","Env3 Decay","Env3 Release","Env3 Amp","LFO1 Speed","LFO1 Amp","LFO2 Speed","LFO2 Amp","Porta Time"],
