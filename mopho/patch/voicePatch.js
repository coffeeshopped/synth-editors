...
const voicePatch = {
  ...
  // var relatedBankType: SysexPatchBank.Type? = FnSingleBank<MophoVoiceBank>.self
  
  initFileName: "Mopho-init",
  
  randomize: function(patch) {
...
voicePatch.paramOptions = PO.reduce(function*() {
  yield* osc()
  yield PO.inc({b: 12}, [
    [["sync"], {p: 10, max: 1}],
  ...
  yield lfo(41, voicePatch)
  yield env3(61, 69, voicePatch)
  yield mods(70, voicePatch)
  yield ctrls(82, voicePatch)
  yield pushIt(92, voicePatch)
  yield tempoArpSeq(95, voicePatch)
  yield PO.prefix(["seq"], {count: 4, bx: 1, px: 1}, [
    [["dest"], {b: 101, p: 77, opts: voicePatch.modDestOptions}],
  ])
  yield seqSteps
  yield PO.prefix(["knob"], {count: 4, bx: 1}, [
    [[], {b: 105, opts: voicePatch.knobAssignOptions}],
  ])
})

voicePatch.params = (function(paramOptions) {
  let p = {}
  ...
  return p
}(voicePatch.paramOptions))


module.exports = voicePatch