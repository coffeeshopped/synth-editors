...
const sysex = bytes => [0xf0, 0x18, 0x04, deviceId, bytes, 0xf7]

const paramSetData = (parm, byte0, byte1) => {
  const pl = parm.bits([0, 6])
  const pm = parm.bits([7, 13])
  return sysex([0x03, pl, pm, byte0, byte1])
}

const paramData = (parm, value) => {
  const vl = value.bits([0, 6])
  const vm = value.bits([7, 13])
  return paramSetData(parm, vl, vm)
}


function pack(parm, value) {
  let off = parm * 2
  guard off + 1 < bytes.count else { return }
  bytes[off] = value.bits(0...6)
  bytes[off + 1] = value.bits(7...13)
}
...
//    }
  ...
  t.extraParamOuts = [
    ("patch", .bankOut("bank", { change, bank in
      ...
      return ["patch/name" : .p([], .options(numberedPatches) <<< [-1 : "Off"])]
    })),
  ]
...
static func paramFetchData(parm: Int) -> [UInt8] {
  sysex([0x02, UInt8(parm.bits(0...6)), UInt8(parm.bits(7...13))])
}
