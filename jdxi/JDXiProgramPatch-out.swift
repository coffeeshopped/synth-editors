...
extension JDXi {
  ...
  enum Program {
    ...
    const patchWerk: RolandMultiPatchTrussWerk = {
    ...
    const rolandMap = {
      var map: [RolandMultiPatchTrussWerk.MapItem] = [
        ...
        ("fx/0", 0x0200, Effect1.patchWerk),
        ("fx/1", 0x0400, Effect2.patchWerk),
        ("delay", 0x0600, Delay.patchWerk),
        ("reverb", 0x0800, Reverb.patchWerk),
        ("ctrl", 0x4000, Ctrlr.patchWerk),
      ]
      map += [
        ("digital/0", 0x2000, Digital1Part.patchWerk),
        ("digital/1", 0x2100, Digital2Part.patchWerk),
      ...
      map += (4..<16).map {
        ...
        return [
          ("part/$0", 0x2000 + off, ExtraPart.patchWerk),
          ("zone/$0", 0x3000 + off, Zone.patchWerk),
        ]
      }.reduce([], +)
      map += 16.map {
        // extra mystery subpatches
        ("extra/0/$0", 0x1000 + 0x0100 * $0 , Extra1.patchWerk) // 15 bytes each
      }
      return map
    }()
    
    const outputAssignOptions = ["EFX1","EFX2","Delay","Reverb","Direct"]
    
    const drumOutAssignOptions: [Int:String] = outputAssignOptions <<< [5 : "Kit"]
    ...
    enum Bank {
      ...
      const bankWerk: RolandMultiBankTrussWerk = {
    ...
    enum Full { 
      ...
      const refTruss = RolandMultiSysexTrussWerk.createFullRefTruss(JDXi.sysexWerk, "Full Program", map, start: 0x18000000, isos: isos, refPath: refPath, initFile: "jdxi-full-program-init", sections: sections)

      const refPath: SynthPath = "perf"

      const map: [RolandMultiSysexTrussWerk.MapItem] = [
        ("perf", 0x00000000, Program.patchWerk),
        ("digital/0", 0x01010000, Digital.patchWerk),
        ("digital/1", 0x01210000, Digital.patchWerk),
        ("analog", 0x01420000, Analog.patchWerk),
        ("rhythm", 0x01700000, Drum.patchWerk),
      ]
            
      const sections: [(String, [SynthPath])] = [
        ("Program", [refPath]),
        ("Digital", 2.map { "digital/$0" }),
        ("Analog", ["analog"]),
        ("Drums", ["rhythm"]),
      ]
...
//      }
    ...
    //  }
      ...
      const isos: FullRefTruss.Isos = partPaths.dict { partPath in
        [partPath : .basic(path: partPath + "bank/lo", location: partPath + "pgm", pathMap: 128.map { bank in
          switch bank {
          case 0, 1, 2, 3:
            return `bank/${partPath.first!}/bank`
          case 64, 65:
            return `preset/${partPath.first!}/bank - 64`
          default:
            return "pgm"
          }
    ...
    private const partPaths: [SynthPath] = ["digital/0", "digital/1", "analog", "rhythm"]
          
    private const usedPaths: [SynthPath] = {
      var paths: [SynthPath] = [
        "common",
        "voice/fx",
        "fx/0",
  ...
  //  }
    ...
    struct Common {
      ...
      const patchWerk = singlePatchWerk("Program Common", params, size: 0x24, start: 0x0000, name: .basic(0..<0x0c), sysexDataFn: sysexData)
      // size 0x1f // DOCS ARE WRONG!
      
      const parms: [Parm] = [
        ['level', { b: 0x0010 }],
        ['tempo', { b: 0x0011, packIso: JDXi.multiPack(0x0011), rng: [500, 30000] }],
        ['voice/fx', { b: 0x0016, opts: ["Off","Vocoder","Auto-Pitch"] }],
        ['voice/fx/number', { b: 0x001c, max: 20, dispOff: 1 }],
        ['voice/fx/part', { b: 0x001d, opts: ["1","2"] }],
        ['auto/note/on', { b: 0x001e, max: 1 }],
      ]
    ...
    const syncRateIso = Miso.options(noteOptions)
  ...
  //      p["vocoder/on"] = RangeParam(byte: 0x000d, maxVal: 1)
    ...
    enum Effect2 {
      ...
      const flangerParams : [(String,Parm.Span)] = [
        ("Rate Sync", lrng(0...1)),
        ("Rate", lrng()),
        ("Rate", syncRateParam),
        ("Depth", lrng()),
      ...
      const phaserParams : [(String,Parm.Span)] = [
        ("Rate Sync", lrng(0...1)),
        ("Rate", lrng()),
        ("Rate", syncRateParam),
        ("Depth", lrng()),
      ...
      const ringParams : [(String,Parm.Span)] = [
        ("Frequency", lrng()),
        ("Sens", lrng()),
        ("Balance", lrng(0...100)),
        ("Level", lrng())
      ...
      const slicerParams : [(String,Parm.Span)] = [
    ...
    enum Delay {
      ...
      const patchWerk = singlePatchWerk("Program Delay", params, size: 0x64, start: 0x0600)
    ...
    enum Reverb {
      ...
      const patchWerk = singlePatchWerk("Program Reverb", params, size: 0x63, start: 0x0800)
    ...
    enum Digital1Part : JDXiPartPatchBuilder {
      ...
      const patchWerk = singlePatchWerk("Program Digital 1", createParams(), size: 0x4c, start: 0x2000)
    ...
    enum Digital2Part : JDXiPartPatchBuilder {
      ...
      const patchWerk = singlePatchWerk("Program Digital 2", createParams(), size: 0x4c, start: 0x2000)
    ...
    enum AnalogPart : JDXiPartPatchBuilder {
      ...
      const patchWerk = singlePatchWerk("Program Analog", createParams(), size: 0x4c, start: 0x2000)
    ...
    enum DrumPart : JDXiPartPatchBuilder {
      const patchWerk = singlePatchWerk("Program Drums", createParams(isDrums: true), size: 0x4c, start: 0x2000)
    ...
    enum ExtraPart : JDXiPartPatchBuilder {
      const patchWerk = singlePatchWerk("Program Extra", createParams(), size: 0x4c, start: 0x2000)
